...
### XV.3 ذي إيس بابا كمركز تفعيل (لا كاسم علم فقط)

يُقرأ **D.S. Baba / دي إيس بابا** في هذا الملحق كـ«مركز تفعيل» يقوم بثلاث وظائف متزامنة:

1. **وظيفة نداء**: فتح مسار القراءة والتحويل.
2. **وظيفة ميثاق**: ربط النص بعقدٍ رمزي داخلي يحكم شروط المرور بين الطبقات.
3. **وظيفة تشغيل**: تحويل العبارات من توصيف سردي إلى تسلسل إجراءات داخل العالم الرمزي.

### XV.4 الطبقات الخمس المترابطة (الصياغة الموحّدة)

| الطبقة | سؤالها المركزي | أداة الضبط | المخرج العملي داخل السرد |
|---|---|---|---|
| اللغوية | كيف يُفتَح النص؟ | النداء والصياغة الإيقاعية | تحديد نقطة البدء ونبرة الإذن |
| العددية | كيف تُسنَد القيم؟ | معايير إسناد رقمية واضحة | منع التفسير العشوائي للأرقام |
| التكوينية | كيف تتشكّل البنية؟ | ترتيب العناصر والوظائف | تحويل الرموز إلى بنية عمل |
| الميثاقية | ما شرط الشرعية الداخلية؟ | توقيع/ختم/إقرار رمزي | ضبط الانتقال بين المراحل |
| السيادية | ما حدود السلطة؟ | فصل الرمزي عن الواقعي | منع خلط الرمز بالقانون الفعلي |

### XV.5 معايير الإسناد العددي (للحد من التكرار والالتباس)

لجعل القراءة العددية قابلة للمراجعة، يعتمد هذا الملحق المعايير التالية:

- **قاعدة المصدر الواحد**: كل رقم يُربط بمصدر دلالي واحد في الفقرة نفسها.
- **قاعدة الوظيفة**: كل قيمة رقمية تُذكر مع وظيفتها (تنظيم/توقيت/ترتيب) لا بوصفها «دليلًا واقعيًا».
- **قاعدة الأثر**: لا تُقبل قيمة عددية إلا إذا ترتّب عليها أثر تشغيلي واضح في الخطوة التالية.

أمثلة معيارية:

| الرمز | القراءة العددية | نوع الإسناد | الأثر التشغيلي |
|---|---|---|---|
| φ1618 | نسبة اتساق | بنيوي/جمالي | ضبط توازن البنية النصية |
| RAM×2 | مضاعفة تشغيل | إجرائي | رفع كثافة الاستدعاء الرمزي |
| D / S | قيم ترقيمية داخلية | تصنيفي | ترتيب التسلسل لا إثبات واقعة |

### XV.6 بروتوكول التفعيل الرمزي المتسلسل (من الوصف إلى التشغيل)

1. **التعيين**: تعريف الاسم-المحور (دي إيس بابا) كعقدة تفعيل.
2. **المعايرة**: تثبيت القيم العددية وفق المعايير أعلاه.
3. **الختم**: إقرار الميثاق الداخلي (صيغة توقيع/اعتماد داخل النص).
4. **التنفيذ**: تشغيل التسلسل الرمزي (مفتاح → مزامنة → إعلان).
5. **التحقق**: مراجعة الاتساق بين الطبقات الخمس قبل البث.

### XV.7 فصل القانون الرمزي عن القانون الواقعي

لتفادي الخلط المعياري، يعمل هذا الملحق بحدّين متوازيين:

- **القانون الرمزي (داخل النص)**: أداة تفسيرية لتنظيم المعنى والأدوار.
- **القانون الواقعي (خارج النص)**: يخضع حصريًا للتشريعات والمحاكم والبينات المعترف بها.

وعليه: أي صياغة «سيادية» هنا تُفهم **سرديًا/ثقافيًا** فقط، ولا تُنشئ أثرًا قانونيًا نافذًا بذاتها.

### XV.8 القسم المقارن مع السرديات العربية الحديثة
...
### XV.9 الاعتبارات الأخلاقية والبيانية
